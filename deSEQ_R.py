...
# Filter out genes that are present in HP126 but missing in R7
...
#count_matrix_filtered <- count_matrix[rowSums(count_matrix[, hp126_samples] > 0) > 0 & rowSums(count_matrix[, r7_samples] > 0) > 0, ]
########################
...
# CODE FOR DISTRIBUTION OF READ COUNTS PER GENE

# Ensure gene_counts is the count matrix (use count_matrix_filtered)
gene_counts <- count_matrix_filtered

# Check the summary statistics of the raw gene counts
summary(gene_counts)

# Check how many zeros are in the data
cat("Number of zeros in gene counts:", sum(gene_counts == 0), "\n")

# Ensure gene_counts is numeric and has variation
cat("Is numeric:", is.numeric(gene_counts), "\n")
cat("Variance of gene counts:", var(gene_counts), "\n")

# Plot density of raw gene counts
plot(density(gene_counts), main = "Density of Raw Gene Counts", xlab = "Gene Counts")

# Log-transform the raw counts (add 1 to avoid log(0))
log_gene_counts <- log2(gene_counts + 1)

# Plot density of log-transformed gene counts with adjusted bandwidth
plot(density(log_gene_counts, bw = 0.1), main = "Density of Log-Transformed Gene Counts", xlab = "Log2(Total Counts + 1) per Gene")

# Plot histogram of log-transformed counts (no filtering)
hist(log2(gene_counts + 1), breaks = 30, main = "Log2-transformed Gene Counts", xlab = "Log2(Total Counts + 1)", col = "skyblue", border = "white")

# Boxplot of gene counts (no filtering)
boxplot(gene_counts, main = "Boxplot of Gene Counts", ylab = "Gene Counts")
